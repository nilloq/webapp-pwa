...
export function useCloudinary() {
  ...
  function getCloudinaryImageUrl(cloudinaryId:string, type:'place' | 'place_thumb' | 'post' = 'place') {
    let transfName = 'mahali_detail_full_screen_watermarked'
    if (type === 'post')
      transfName = 'mahali_post'
    else if (type === 'place_thumb')
      transfName = 'mahali_detail_thumb_watermarked'

    return `https://res.cloudinary.com/mahali/image/private/s--bcOIh4fh--/t_${transfName}/v1/${cloudinaryId}`
  }

  function getCloudinaryShortLink(url:string, type:'place' | 'place_thumb' | 'post' = 'place'):string {
    let transfName = 'mahali_detail_full_screen_watermarked'
    if (type === 'post')
      transfName = 'mahali_post'
    else if (type === 'place_thumb')
